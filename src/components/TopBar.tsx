...
function TopBar() {
  ...
  const appWindow = useMemo(() => (isMobile ? null : getCurrentWindow()), [isMobile]);
  const dragRegionStyle: CSSProperties & { WebkitAppRegion?: string } = isMobile
    ? {}
    : {
  ...
  return (
    <header
      ...
      data-tauri-drag-region={isMobile ? undefined : true}
      style={dragRegionStyle}
      onDoubleClick={() => {
        if (!isMobile && appWindow) {
