      ...
      <main className="relative mx-auto max-w-6xl space-y-6 px-4 py-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-tertiary)]">Proxmark · NTAG216</p>
            <h1 className="font-pp-machina text-3xl leading-tight text-[var(--text-primary)]">NTAG216 JSON Studio</h1>
            <p className="text-sm text-[var(--text-tertiary)]">
              Modern desk UI to auto-detect your Proxmark, validate payload sizing, and read/write{" "}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="greenTint" onClick={readJsonTag} disabled={busy}>
        ...
        >
          <CardHeader className="gap-3">
            ...
            <CardDescription>
              If a single serial device is present we'll select it automatically. Otherwise pick the Proxmark below.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-[2fr_1fr]">
            <div className="space-y-3">
              ...
              <Select
                ...
                className="h-11 rounded-lg border-[var(--brand-light-green)]/40 bg-[var(--brand-dark-green)] text-[var(--text-primary)]"
              >
                <option value="">Auto-detect</option>
                {ports.map((p) => {
                  const label =
                    p.product || p.manufacturer ? `${p.product ?? p.manufacturer} — ${p.port_name}` : p.port_name;
                  return (
              ...
              <div className="text-xs text-[var(--text-tertiary)]">
                Selected: <span className="font-mono text-[var(--text-primary)]">{portLabel}</span>
              </div>
            </div>
            <div className="flex items-end justify-end gap-2">
              <Button variant="secondary" onClick={refreshPorts} className="w-full md:w-auto">
                {busy ? "Working…" : "Refresh"}
              </Button>
            </div>
            <div className="md:col-span-2 rounded-lg border border-[var(--brand-light-green)]/25 bg-[var(--brand-light-dark-green)] px-3 py-2 text-sm text-[var(--text-primary)]">
